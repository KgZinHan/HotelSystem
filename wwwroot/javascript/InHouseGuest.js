...
// Guest Info
function chooseGuest(checkInId) {


    $.ajax({
        type: 'GET',
        url: "/InHouseGuest/GuestInfo",
...
$('#inHouseGuestForm').submit(function (event) {
    event.preventDefault();
    const shouldProceed = window.confirm("Are you sure of saving record?");
    if (shouldProceed) {

        const checkInId = $('#inputCheckInId').val();
...
function changeStateSelectList(countryId, guestId) {
    ...
    $.ajax({
        type: 'GET',
        url: "/InHouseGuest/ChangeStateSelectList",
...
function createNationality() {
    ...
    $.ajax({
        type: 'GET',
        url: "/InHouseGuest/CreateNationality",
